...
Идентификационные и журнальные БД (`sm_identity.db`, `sm_messages.db`) появятся автоматически рядом с бинарником, если не указать другое местоположение.
...
1. На каждом устройстве установите Qt 6.5+ и соберите демонстрационный клиент (опционально):
   ...
   ```
   Текущий QML-клиент показывает работу интерфейса и журналов, но сетевое взаимодействие можно реализовать, подключив gRPC-слой к `AppController`.
