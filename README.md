...
## 11. Криптография и защита трафика
...
- Шифрование полезной нагрузки HTTP: поля тел сообщений дополнительно шифруются симметричным ключом AES-256-GCM. Ключ задаётся переменной `SM_MESSAGE_KEY` (или флагом `--message-key`) как base64-строка длиной 32 байта; nonce генерируется случайно и приставляется к ciphertext.
- Токены сессии HTTP: после мTLS-аутентификации сервер выдаёт bearer-токены для повторного доступа. Они генерируются из 32 байтов криптографически стойких случайных данных, кодируются `base64url` и действуют по умолчанию 30 минут.
